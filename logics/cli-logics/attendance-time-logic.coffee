...
# クライント側、開始時刻設定ロジッククラス
#
class AttendanceTimeLogic extends BaseLogic

  _accessToken = null

  # AttendanceTimeLogicを生成するコンストラクタ
  #
  # @param [Object] _logger Log4jsログオブジェクト
  #
  constructor: (logger) ->
    logger.debug 'AttendanceTimeLogic#constructor'
    super logger
  ...
  validate: (request, callback) ->
    @_logger.debug 'AttendanceTimeLogic#validate'
  ...
  execute: (request, callback) ->
    @_logger.debug 'AttendanceTimeLogic#execute'
    db = @getDBManager()
    db.open (err, client) =>
      if err
        ...
        Async.waterfall [
          (asyncCallback) =>
            ...
            db.begin (err) ->
              asyncCallback err
          (asyncCallback) =>
            @_logger.debug 'Set AttendanceTime'
            dutyTimeModel.setAttendanceTime user.userid, (err) ->
              asyncCallback err
          (asyncCallback) =>
